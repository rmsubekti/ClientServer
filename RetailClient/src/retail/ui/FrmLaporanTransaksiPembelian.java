 ...
 * @author su
 */
public class FrmLaporanTransaksiPembelian extends javax.swing.JInternalFrame {
    private final Object[] transaksiColumnNames = {
        "No","Nota","Supplier","Tanggal"
    };
    private DefaultTableModel tableModelTransaksi = new DefaultTableModel();
    private List<Beli> recordTransaksi = new ArrayList<> ();
    private IBeliDao transaksiDao = null;
    ...
    private List<ItemBeli> recordItem = new ArrayList<> ();
    private IItemBeliDao itemDao = null;
    /**
     * Creates new form FrmLaporanTransaksiPembelian
     */
    public FrmLaporanTransaksiPembelian() {
        initComponents();
        initTableItem();
        initTableTransaksi();
        initBeliDao();
    ...
    private void initBeliDao() {
        ...
        try {
            ...
            transaksiDao = (IBeliDao) Naming.lookup(url);
    ...
    private void initItemBeliDao() {
        ...
        try {
            // ambil referensi dari remote object yg ada di server
            itemDao = (IItemBeliDao) Naming.lookup(url);
        } catch (NotBoundException ex) {
            Logger.getLogger(FrmBarang.class.getName()).log(Level.SEVERE, null, ex);
        } catch (MalformedURLException ex) {
            Logger.getLogger(FrmBarang.class.getName()).log(Level.SEVERE, null, ex);
    ...
    try {
            ...
            for (Beli t : recordTransaksi) {
                System.out.println(t.getNotaBeli());
                // ambil nomor urut terakhir
                int noUrut = tableModelTransaksi.getRowCount() + 1;
                Object[] objects = new Object[transaksiColumnNames.length];
                objects[0] = noUrut;
                objects[1] = t.getNotaBeli();
                objects[2] = t.getSupplier().getNamaSupplier();
                objects[3] = t.getTanggal();
